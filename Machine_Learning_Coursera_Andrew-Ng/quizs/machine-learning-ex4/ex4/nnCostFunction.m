...
% Do not regularizing the terms of bias
t1 = Theta1(:,2:end);
t2 = Theta2(:,2:end);
...
%               first time.
%
% delta_3 = A2' - yk; % 10 x 5000
% delta_2 = (delta_3' * Theta2) .* sigmoidGradient(a2); % 5000 x 26
% delta_2 = delta_2(:,2:end); % 5000 x 25

for t = 1:m,
    a1 = X(t,:);    % X already have bias; 1 x 401
    z2 = Theta1 * a1'; % 25 x 1
    a2 = sigmoid(z2); % 25 x 1
    a2 = [1; a2];   % add bias: 26 x 1
    z3 = Theta2 * a2; % 10 x 1
    a3 = sigmoid(z3);  % output layer: 10 x 1
    
    % back porpagation
    z2 = [1; z2];
    
    delta_3 = a3 - yk(:,t); % 10 x 1
    delta_2 = (Theta2' * delta_3) .* sigmoidGradient(z2);   % 26 x 1
    
    delta_2 = delta_2(2:end);   % skip delta_2(0): 25 x 1
